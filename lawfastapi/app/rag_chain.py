...
class RAGChain:
    ...
    async def question_checker(self, state: GraphState) -> GraphState:
        ...
        response = await chain.ainvoke(
            {"question": state["question"], "chat_history": formatted_history}
        )
        question_check = "notGrounded"
        if response == "yes":
            question_check = "grounded"
        state["relevance"] = question_check
    ...
    def not_found_in_context(self, state: GraphState) -> GraphState:
        ...
        state["answer"] = (
            "죄송합니다. 현재 제가 가진 정보로는 말씀하신 내용에 대해 정확한 답변을 드리기 어렵습니다. 혹시 고용노동법과 관련된 다른 궁금하신 점이 있으신가요?"
        )
        return state
    ...
    async def rewrite(self, state: GraphState) -> GraphState:
        ...
        state["is_rewrite"] = True
        return state

    def is_relevant(self, state: GraphState) -> str:
        if state["is_rewrite"] == True:
            return "notFound"
